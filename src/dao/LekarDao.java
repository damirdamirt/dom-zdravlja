...
public class LekarDao {
		...
		try {
			...
			while ((linija = reader.readLine()) !=null) {
				...
				Lekar lekar = new Lekar(ime, prezime, jmbg, brTel, uloga, adresa, korIme, 
										lozinka, p, plata, s, spec, pregledi);
				lekari.add(lekar);
	...
	public void izbrisiLekara(Lekar obrisaniLekar) {
		ucitajLekare();
		ArrayList<Lekar> preostaliLekari = new ArrayList<Lekar>();
		for (Lekar lekar : lekari ) {
			if (!obrisaniLekar.getKorIme().equals(lekar.getKorIme())) {
				preostaliLekari.add(lekar);
			}
		}
