...
public class ZdrKnjizicaDao {
	...
	public void ucitajZdrKnjizice() {
		...
		try {
			...
			while ((linija = reader.readLine()) != null) {
				...
				int katOs = Integer.valueOf(podaci[2]).intValue();
				KategOsiguranjaKnjizice katOsig = KategOsiguranjaKnjizice.getOsiguranje(katOs);
	...
	public void upisiKnjizicu(ZdrKnjiz knjiz) {
		...
		try {
			...
			String linija = knjiz.getBroj() + "|" + datum + "|" + knjiz.getKatOsig().getBroj() + "\n";
			writer.write(linija);
			writer.close();
	...
	public void upisiKnjizice(ArrayList<ZdrKnjiz> knjizice) {
		...
		try {
			...
			for (ZdrKnjiz knj : knjizice) {
				...
				String linija = knj.getBroj() + "|" + datum + "|" + knj.getKatOsig().getBroj() + "|" + "\n";
				writer.write(linija);
			}
			writer.close();
	...
	public boolean validacijaIdknjizice(String broj) {
		ucitajZdrKnjizice();
		boolean trazenaKnjizica = true;
		for (ZdrKnjiz knjizica : knjizice) {
			if (broj.equals(knjizica.getBroj())) {
				trazenaKnjizica = false;
				break;
			}
		}
		return trazenaKnjizica;
	}
}
